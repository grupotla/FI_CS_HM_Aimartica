...
<br>
<center>
<p><h1>- Buscar Puerto Descarga -</h1>
...
//$result = pg_query($link2, "SELECT unlocode_id, codigo, nombre, pais FROM unlocode WHERE pais = 'CR' AND nombre LIKE '%$keyword2%' ORDER BY nombre");

$sql = "SELECT unlocode_id, codigo, nombre, pais FROM unlocode WHERE pais = 'CR' AND nombre ILIKE '%$keyword2%' ORDER BY nombre";
...
if ($rs['recordset']) 
{
	...
	for ($row = 0; $row < pg_num_rows($rs['recordset']); $row++) 
	{
		...
		printf('<tr $color1><td> &nbsp;%s&nbsp; </td><td> &nbsp;%s&nbsp; </td><td> &nbsp;%s&nbsp; </td><td> &nbsp;%s&nbsp; </td></tr>', 
            //"<a class=labellist href=# onclick=\"top.opener.document.forms[0].puerto_destino.value = '$codigo'; top.close();\">$codigo</a>",
            $row + 1 + (($rs['curPage']-1)*$rs['lineasxpagina']),
            "<a class=labellist href='$return1&puerto_destino=$codigo'>$codigo</a>",        
            $nombre, $pais);
    }
}
?>       
