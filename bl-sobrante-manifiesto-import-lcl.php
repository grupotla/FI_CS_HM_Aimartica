...
$query_xml = "SELECT xml_enviado FROM manifiestos WHERE viaje = TRIM('$vuelo') AND operacion_id = 11 AND import_export = '{$_SESSION['ie']}'";
...
$sql = pg_exec($link, "INSERT INTO manifiestos (sistema_id, correlativo, operacion_id, viaje, fecha_envio, enviado_por, fecha_original, xml_original, original_por, import_export) VALUES (2, '".$correlativo_aplicar."', 11, '".$vuelo."', NOW(), '".$nombres."', '".$fecha_original."', '".$xml_original."', '".$enviado_original."','{$_SESSION['ie']}')");
...
//	$row3 = 5000;
	...
	for ($row = 1; $row <= pg_num_rows($result); $row++)
	{
		...
		if ($transmitido == 'f' and $activo == 't' and $asociated == 'f' and $transmitido2 == 'f')
		{
			if ($secuencia1 == "1")
			{
				$row5++; // sustituye a $row
				$row7++; // sustituye a $row3
$buffer_bl = $buffer_bl.
'
                          <ROW_YCGCON NUM = "'.$row5.'" >
                              ...
                              <YCGFCHEND></YCGFCHEND>
                                 <ROWSET_YCGLIN>
                                     <ROW_YCGLIN NUM = "1" >
                                         ...
                                         <YCGTXTACTA>ASOCIACION DE NUEVO BL CON CONTENEDOR EXISTENTE</YCGTXTACTA>
                                            <ROWSET_YCGCNTLI>
                                                <ROW_YCGCNTLI NUM = "1" >
...
';
		...
		$sql = "UPDATE manifiestos_counter SET sec_tica = $row5 WHERE contenedor = '$contenedor'";
...
//	$row4 = 5000;
	...
	$query_desembarque = pg_fetch_array($puerto_desembarque, 0);
		...
		if ($transmitido == 'f' and $activo == 't' and $asociated == 't' and $transmitido2 == 'f')
		{
			if ($secuencia2 == "1")
			{
				$row6++; // sustituye a $row
				$row8++; // sustituye a $row3
$buffer_bl = $buffer_bl.
'
                          <ROW_YCGCON NUM = "'.$row6.'" >
                              <YCGSECCON>'.$row8.'</YCGSECCON>
...
';
		...
		$row6++; // sustituye a $row2
		$row8++; // sustituye a $row4
		...
		$sql = "UPDATE manifiestos_counter SET sec_tica = $row6 WHERE contenedor = '$contenedor'";
		$resultado = pg_exec($link, $sql);
...
if ($sql)
{
	$sql = "UPDATE manifiestos SET xml_enviado = '$name_file' WHERE correlativo = $correlativo_aplicar";
	$resultado = pg_exec($link, $sql);
	$sql = "COMMIT";

	if (pg_num_rows($result) > 0)
	{
		$sql = "UPDATE bill_of_lading SET asociated = true WHERE bl_id = $blid AND import_export = '{$_SESSION['ie']}'";
		$resultado = pg_exec($link, $sql);
		$sql = "COMMIT";
	}
	...
	if (pg_num_rows($result2) > 0)
	{
		$sql = "UPDATE divisiones_bl SET asociated = true WHERE division_id = $hdivision";
		$resultado = pg_exec($link, $sql);
		$sql = "COMMIT";
	}
