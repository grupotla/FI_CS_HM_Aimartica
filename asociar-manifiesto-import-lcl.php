...
$query_xml = "SELECT xml_enviado FROM manifiestos WHERE viaje = TRIM('$vuelo') AND operacion_id = 9 AND import_export = '{$_SESSION['ie']}'";
...
$sql = pg_exec($link, "INSERT INTO manifiestos (sistema_id, correlativo, operacion_id, viaje, fecha_envio, enviado_por, fecha_original, xml_original, original_por, import_export) VALUES (2, '".$correlativo_aplicar."', 9, '".$vuelo."', NOW(), '".$nombres."', '".$fecha_original."', '".$xml_original."', '".$enviado_original."','{$_SESSION['ie']}')");
...
if ($result)
{
	...
	for ($row = 1; $row <= pg_num_rows($result); $row++)
	{
		...
		if ($transmitido == 'f' and $activo == 't' and $asociated == 'f')
		{
...
'
			<ROWSET_YCGCNTDR>
				<ROW_YCGCNTDR NUM="1">
					<YCGNROCONT>'.$contenedor4.'</YCGNROCONT>
		...
		if ($transmitido == 't' and $activo == 'f')
		{

$buffer_bl = $buffer_bl.
'
                          <ROW_YCGCON NUM = "'.$row.'" >
                              <YCGSECCON>'.$row2.'</YCGSECCON>
                                 <ROWSET_YCGLIN>
                                     <ROW_YCGLIN NUM = "1" >
...
';
		...
		{
			$values = pg_fetch_array($result2, $row4, PGSQL_ASSOC);
		}
...
fwrite($file, $buffer_viaje.$buffer_contenedor.$buffer_bl.$buffer_linea.$buffer_xml);
fclose($file);
