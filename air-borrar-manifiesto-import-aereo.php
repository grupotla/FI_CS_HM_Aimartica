...
{
	$arrivaldate = cambia_a_mysql($row["arrivaldate"];
	$vuelo = $row["vuelo"];
...
$query_hwb = mysqli_query($link,"SELECT b.awbid, b.hawbnumber, b.consignerdata, b.consignerid, b.natureqtygoods, b.totweight, b.totnoofpieces, m.puerto_origen, m.puerto_destino, m.bultos, m.almacen, m.fecha_endoso, b.statusmanif FROM {$_SESSION['air']} b, manifiestos_import m WHERE b.awbnumber = m.awbnumber AND b.hawbnumber = m.hawbnumber AND m.parcializado = 0 AND b.awbnumber = '$awb' AND import_export = '{$_SESSION['ie']}'");
