...
$tipced = "";
$query = mysqli_query($link,"SELECT vuelo, conocimiento, ubicacion, fecha_arribo_tica, cedula2 FROM manifiestos_import WHERE parcializado = 1 AND id = $id AND awbnumber = '$awb' AND import_export = '{$_SESSION['ie']}'");
...
$query_hwb = mysqli_query($link,"SELECT m.hawbnumber, b.consignerdata, b.consignerid, b.natureqtygoods, m.peso, m.piezas, m.puerto_origen, m.puerto_destino, m.bultos, m.almacen, m.fecha_endoso, m.statusmanif FROM {$_SESSION['air']} b, manifiestos_import m WHERE b.awbnumber = m.awbnumber AND b.hawbnumber = m.hawbnumber AND b.expired = 0 AND m.parcializado = 1 AND m.awbnumber = '$awb' AND m.id = $id AND import_export = '{$_SESSION['ie']}'");
...
$buffer_contenedor = '<ROW_YCGCON NUM = "'.$row3.'" >
                              ...
                              <YCGNOMCONS>'.strtoupper(trim($consignatario3[0])).'</YCGNOMCONS>
                              <YCGFCHEND></YCGFCHEND>
...
if ($sql)
{
	...
	$sql = "UPDATE manifiestos_import SET transmited = 1 WHERE awbnumber = '$awb' AND id = $id AND import_export = '{$_SESSION['ie']}' ";
