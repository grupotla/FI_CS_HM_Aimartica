...
/*
$sql_embalaje = "SELECT identificador AS bultos, descripcion FROM embalajes_tica ORDER BY bultos";
...
$sql = "SELECT identificador AS bultos, descripcion FROM embalajes_tica WHERE descripcion ILIKE '%$keyword2%' and descripcion <> '' ORDER BY descripcion";
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tbl-regs">
    <tr>
      ...
      <th "style='background-color: #3399FF; color: #FFFFFF;'" align="center" width="7%">Embalaje</th>
    </tr>
<?php
if ($rs['recordset']) 
{
	...
	for ($row = 0; $row < pg_num_rows($rs['recordset']); $row++) 
	{
      ...
      "<a class=labellist href='$return1&bultos=$codigo'>$codigo</a>",
      $nombre);
    }
}
?>       
